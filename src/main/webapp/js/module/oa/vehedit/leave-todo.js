...
function loadDetail(id, withVars, callback) {
    var dialog = this;
    $.getJSON(ctx + '/oa/vehedit/detail/' + id, function(data) {
        detail = data;
...
function loadDetailWithTaskVars(leaveId, taskId, callback) {
    var dialog = this;
    $.getJSON(ctx + '/oa/vehedit/detail-with-vars/' + leaveId + "/" + taskId, function(data) {
        detail = data;
        $.each(data, function(k, v) {
            // 格式化日期
			if (k == 'applyTime' || k == 'startTime' || k == 'endTime') {
...
function complete(taskId, variables) {
	...
	// 发送任务完成请求
    $.post(ctx + '/oa/vehedit/complete/' + taskId, {
        keys: keys,
        values: values,
        types: types
    }, function(resp) {
...
var handleOpts = {
		...
		btns: [{
			...
			click: function() {
				...
				// 设置流程变量
				complete(taskId, [{
					key: 'countyDeptLeaderPass',
					value: 1,
					type: 'A'
				},{
					key:'shigssp',
					value:'kafeitu,leaderuser'
				}
